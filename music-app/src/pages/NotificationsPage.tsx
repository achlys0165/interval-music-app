...
const NotificationsPage: React.FC = () => {
  ...
  const getIcon = (message: string) => {
    ...
    if (message.includes('rejected')) return <AlertCircle size={18} className="text-red-500" />;
    if (message.includes('assigned')) return <Calendar size={18} className="text-blue-500" />;
      ...
      <div className="space-y-4">
        {myNotifications.length > 0 ? (
          myNotifications.sort((a: any, b: any) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime()).map((notif: any) => (
            ...
            >
              ...
              <div className={`shrink-0 p-4 rounded-3xl ${notif.read ? 'bg-white/5' : 'bg-white/10 group-hover:scale-110 transition-transform duration-500'}`}>
                {getIcon(notif.message.toLowerCase())}
              </div>
                ...
                <div className="flex items-center justify-between gap-4 mb-2">
                   ...
                   {!notif.read && (
                     <span className="flex items-center gap-1.5 text-[9px] font-black uppercase tracking-widest text-white/40">
                       <Circle className="fill-white text-white" size={6} /> New Activity
                     </span>
                   )}
                </div>
          ...
          <div className="py-32 text-center border-2 border-dashed border-white/5 rounded-[3rem] flex flex-col items-center">
            ...
            <p className="text-white/20 italic text-lg">Your dashboard is currently quiet.</p>
            <p className="text-white/10 text-[10px] uppercase tracking-widest mt-2">New activities will appear here</p>
          </div>
        )}
      </div>
    </div>
