...
import { MOCK_USERS, CHROMATIC_SCALE } from '../constants';
import { 
  UserPlus, 
  PlusCircle, 
  CheckCircle, 
...
const AdminPanel: React.FC = () => {
  const { songs, setlists, schedules, addSong, assignMusician, updateSetlist } = useData();
  const [activeTab, setActiveTab] = useState<'roster' | 'assign' | 'planner' | 'songs'>('roster');
  
  // Form States
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [newSong, setNewSong] = useState({ title: '', originalKey: 'C', category: 'Worship' as any, lyrics: '', referenceUrl: '' });
  ...
  const handleAddSong = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      addSong(newSong);
      setNewSong({ title: '', originalKey: 'C', category: 'Worship', lyrics: '', referenceUrl: '' });
  ...
  const handleSaveSetlist = (e: React.FormEvent) => {
    ...
    setIsSubmitting(true);
    setTimeout(() => {
      updateSetlist({ date: plannerDate, songIds: plannerSongs });
      setIsSubmitting(false);
    }, 500);
  };

  const toggleSongInPlanner = (id: string) => {
    setPlannerSongs(prev => prev.includes(id) ? prev.filter(sid => sid !== id) : [...prev, id]);
  };
                   ...
                   return (
                     ...
                     >
                       <p className="text-[9px] uppercase font-black tracking-tighter opacity-60">
                         {new Date(y, m-1, d).toLocaleDateString(undefined, { weekday: 'long' })}
                       </p>
          ...
          {activeTab === 'planner' && (
            <div className="bg-[#0a0a0a] border border-white/10 rounded-3xl p-8 space-y-6">
              ...
              <form onSubmit={handleSaveSetlist} className="space-y-4">
                <div className="space-y-2">
                  <label className="text-[10px] uppercase tracking-widest text-white/40 font-bold ml-1">Service Date</label>
                  <select className="w-full bg-black border border-white/10 rounded-xl py-3 px-4 text-sm" value={plannerDate} onChange={(e) => {
                    setPlannerDate(e.target.value);
                    const existing = setlists.find(s => s.date === e.target.value);
                    setPlannerSongs(existing?.songIds || []);
                  }} required>
                ...
                <div className="space-y-2">
                  ...
                  <div className="max-h-48 overflow-y-auto space-y-1 pr-2">
                    {songs.map(s => (
                      <button type="button" key={s.id} onClick={() => toggleSongInPlanner(s.id)} className={`w-full text-left px-3 py-2 rounded-lg text-xs font-bold flex items-center justify-between transition-colors ${plannerSongs.includes(s.id) ? 'bg-white text-black' : 'bg-white/5 text-white/60 hover:bg-white/10'}`}>
                        {s.title}
            ...
            <div className="bg-[#0a0a0a] border border-white/10 rounded-3xl p-8 space-y-6">
              ...
              <form onSubmit={handleAddSong} className="space-y-4">
                <div className="space-y-2">
                  ...
                  <input type="text" placeholder="Title" className="w-full bg-black border border-white/10 rounded-xl py-3 px-4 text-sm" value={newSong.title} onChange={(e) => setNewSong({...newSong, title: e.target.value})} required />
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <select className="bg-black border border-white/10 rounded-xl py-3 px-4 text-sm" value={newSong.originalKey} onChange={(e) => setNewSong({...newSong, originalKey: e.target.value})}>
                    {CHROMATIC_SCALE.map(k => <option key={k} value={k}>{k}</option>)}
                  </select>
                  <select className="bg-black border border-white/10 rounded-xl py-3 px-4 text-sm" value={newSong.category} onChange={(e) => setNewSong({...newSong, category: e.target.value as any})}>
                    <option value="Worship">Worship</option><option value="Choir">Choir</option><option value="Special">Special</option>
                  </select>
                </div>
                <button disabled={isSubmitting} className="w-full py-4 bg-white text-black rounded-xl font-bold uppercase text-xs tracking-widest flex items-center justify-center gap-2">
                  Add to Vault
                ...
                <div className="flex flex-col h-full">
                   ...
                   <div className="flex-1 p-8 grid grid-cols-1 md:grid-cols-2 gap-6 content-start">
                      {ROLES.map(role => {
                        ...
                        return (
                          <div key={role} className={`p-6 rounded-3xl border transition-all ${
                            assignment 
                            ...
                            <div className="flex items-start justify-between">
                              ...
                              {!assignment && (
                                <button 
                                  onClick={() => { setActiveTab('assign'); setNewAssignment(prev => ({ ...prev, role, date: selectedRosterDate })); }}
                     ...
                     <div key={s.id} className="p-6 flex items-center justify-between hover:bg-white/[0.02] transition-colors">
                        <div className="flex items-center gap-6">
                           <div className="text-center w-12">
                             <p className="text-[9px] uppercase font-black text-white/20">{new Date(s.date).toLocaleDateString('en-US', { month: 'short' })}</p>
                             <p className="text-2xl font-black italic">{new Date(s.date).getDate()}</p>
                           </div>
                           <div className="w-px h-8 bg-white/5" />
                           <div>
                              <p className="font-bold text-lg">{MOCK_USERS.find(u => u.id === s.musicianId)?.name}</p>
                   ...
                   {activeTab === 'songs' && songs.slice().reverse().map(song => (
                     <div key={song.id} className="p-6 flex items-center justify-between hover:bg-white/[0.02] transition-colors group">
                       <div>
                         ...
                         <p className="text-[10px] uppercase text-white/30 font-bold tracking-widest">{song.category} • {song.originalKey}</p>
                       </div>
                   ...
                   {activeTab === 'planner' && setlists.slice().reverse().map(sl => (
                     <div key={sl.id} className="p-8 flex items-center justify-between hover:bg-white/[0.02] transition-colors">
                        <div className="flex items-center gap-6">
                           ...
                           <div>
                              <p className="text-xl font-bold">{new Date(sl.date).toLocaleDateString(undefined, { month: 'long', day: 'numeric', year: 'numeric' })}</p>
                              <p className="text-[10px] uppercase tracking-[0.2em] font-black text-white/30">{sl.songIds.length} Songs Loaded</p>
                           </div>
                        </div>
                        <button onClick={() => { setPlannerDate(sl.date); setPlannerSongs(sl.songIds); }} className="p-3 bg-white/5 hover:bg-white hover:text-black rounded-xl transition-all">
                           <Calendar size={18} />
                        </button>
                     </div>
                   ))}
