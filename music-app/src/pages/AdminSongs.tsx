        ...
        <div className="bg-[#0a0a0a] border border-white/20 rounded-[2.5rem] p-8 animate-in fade-in slide-in-from-top-4 duration-500 shadow-2xl max-w-2xl mx-auto w-full">
           ...
           <form onSubmit={handleAddSong} className="space-y-6">
              <div className="space-y-2">
                ...
                <input type="text" placeholder="e.g. Way Maker" className="w-full bg-black border border-white/10 rounded-2xl py-4 px-5 text-sm outline-none focus:border-white/40 transition-all" value={newSong.title} onChange={(e) => setNewSong({...newSong, title: e.target.value})} required />
              </div>
              <div className="grid grid-cols-2 gap-6">
                <div className="space-y-2">
                   <label className="text-[10px] uppercase tracking-widest text-white/40 font-bold ml-1">Original Key</label>
                   <select className="w-full bg-black border border-white/10 rounded-2xl py-4 px-5 text-sm outline-none cursor-pointer" value={newSong.originalKey} onChange={(e) => setNewSong({...newSong, originalKey: e.target.value})}>
                ...
                <div className="space-y-2">
                   ...
                   <select className="w-full bg-black border border-white/10 rounded-2xl py-4 px-5 text-sm outline-none cursor-pointer" value={newSong.category} onChange={(e) => setNewSong({...newSong, category: e.target.value as any})}>
              ...
              <div className="space-y-2">
                 ...
                 <textarea rows={6} className="w-full bg-black border border-white/10 rounded-2xl py-4 px-5 text-sm outline-none focus:border-white/40 resize-none font-mono" value={newSong.lyrics} onChange={(e) => setNewSong({...newSong, lyrics: e.target.value})} />
              </div>
              <button disabled={isSubmitting} className="w-full py-4 bg-white text-black rounded-2xl font-black uppercase text-xs tracking-[0.2em] flex items-center justify-center gap-3">
                 ...
                 <div key={song.id} className="p-8 flex items-center justify-between hover:bg-white/[0.02] transition-colors group">
                    <div className="flex items-center gap-6">
                       ...
                       <div>
                          ...
                          <p className="text-[10px] uppercase tracking-widest font-black text-white/20 mt-0.5">{song.category} • Original Key: {song.originalKey}</p>
                       </div>
                    </div>
