...
const SongDetailModal: React.FC<SongDetailModalProps> = ({ song, onClose }) => {
  ...
  const currentKey = useMemo(() => {
    if (!song) return 'C';
    const originalIndex = CHROMATIC_SCALE.indexOf(song.originalKey);
      ...
      <div className="relative bg-[#0a0a0a] border border-white/10 w-full max-w-4xl max-h-[90vh] rounded-3xl overflow-hidden flex flex-col shadow-2xl">
        <div className="p-6 border-b border-white/5 flex items-center justify-between bg-black">
          <div>
            ...
            <p className="text-xs text-white/40 uppercase tracking-widest mt-1">{song.category} • Original Key: {song.originalKey}</p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto grid grid-cols-1 lg:grid-cols-3">
          <div className="p-6 border-b lg:border-b-0 lg:border-r border-white/5 bg-[#050505] flex flex-col gap-8">
            <div className="text-center space-y-4">
              ...
              <div className="w-20 h-20 bg-white text-black rounded-full flex items-center justify-center text-3xl font-black mx-auto shadow-2xl">{currentKey}</div>
              <div className="flex items-center justify-center gap-3">
                <button onClick={() => setTransposeOffset(prev => prev - 1)} className="p-2 border border-white/10 rounded-lg hover:bg-white hover:text-black transition-all"><Minus size={18} /></button>
                <span className="text-sm font-bold min-w-[40px]">{transposeOffset > 0 ? `+${transposeOffset}` : transposeOffset}</span>
                <button onClick={() => setTransposeOffset(prev => prev + 1)} className="p-2 border border-white/10 rounded-lg hover:bg-white hover:text-black transition-all"><Plus size={18} /></button>
              </div>
              <button onClick={() => setTransposeOffset(0)} className="text-[10px] uppercase font-bold text-white/20 hover:text-white flex items-center gap-2 mx-auto pt-2"><RotateCcw size={12} /> Reset to Original</button>
            </div>
            ...
            <div className="space-y-4">
              <div className="p-4 bg-white/5 rounded-2xl border border-white/10 space-y-3">
                ...
                {song.referenceUrl ? (
                  <a href={song.referenceUrl} target="_blank" rel="noreferrer" className="flex items-center justify-between w-full p-3 bg-white text-black rounded-xl font-bold text-xs uppercase tracking-widest hover:bg-white/90 transition-all">
            ...
            {/* Metronome Integration - Passes song title for AI analysis */}
            <Metronome initialBpm={songBpm} songTitle={song.title} />
          </div>

          <div className="lg:col-span-2 p-8 bg-black/40 relative">
            <div className="flex items-center justify-between mb-6">
               <h3 className="font-bold flex items-center gap-2 italic uppercase"><Music size={18} className="text-white/40" /> Performance Sheet</h3>
               <button onClick={() => navigator.clipboard.writeText(transposedSheet)} className="text-[10px] font-bold uppercase tracking-widest flex items-center gap-2 hover:text-white text-white/30 transition-colors"><Copy size={12} /> Copy Transposed</button>
            </div>
            <div className="bg-[#050505] p-6 rounded-2xl border border-white/5 h-full min-h-[400px]">
              <pre className="font-mono text-sm leading-relaxed text-white/80 whitespace-pre-wrap">{transposedSheet || "No content."}</pre>
            </div>
          </div>
        </div>
      </div>
